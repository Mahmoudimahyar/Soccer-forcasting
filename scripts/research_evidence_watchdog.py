"""Self-supervising watchdog for the durable Evidence-Power Consolidation run. Runs every 10 min (via Task
Scheduler), independently of any chat session. Single GLOBAL LOCK -> never launches concurrent controllers
(never a 2nd worker). NEVER modifies WorldCupShadowCollector / the active collector. Reads only run
state/heartbeat/job_log + collector heartbeat + Task Scheduler state. Writes only watchdog_log.jsonl /
watchdog_state.json / EVIDENCE_POWER_CONSOLIDATION_OPERATIONS_LOG.md. research_only.

Restart policy: restarts ONLY a crashed/stale run (task not Running + heartbeat stale + a job stuck
'running' + collector healthy). NEVER restarts a terminal clean run (queue_complete / all jobs terminal).
Mirrors scripts/research_truth_fusion_watchdog.py.
"""
...
MAIN_TASK = "WorldCupEvidencePowerConsolidationRun"
WATCHDOG_TASK = "WorldCupEvidencePowerConsolidationWatchdog"
...
def _run_terminal(state, summary):
    """A run is terminal-clean iff a run_summary exists with stop_reason queue_complete, OR every job is
    in a terminal status (complete/skipped/blocked) with none left 'running'."""
...
def main():
    ...
    try:
        ...
        with (ROOT / "notes/research/EVIDENCE_POWER_CONSOLIDATION_OPERATIONS_LOG.md").open(
